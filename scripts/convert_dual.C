...
void convert(const char* fname, int nevents = 1, int startevt = 0)
{
	...
	for ( int iplane = 1; iplane < 3; ++iplane) {
	    ...
	    // Split collection plane into two sides of TPC
	    // Glue U and V views with oposit time direction
	    auto h = new TH2F(Form("hRawDigitsVsWireVsTick_plane%d_%d", iplane, ievent), ";Ticks;Wire;ADC",
			      (iplane==2)?cols:2*cols+80, (iplane==2)?-cols/2:-cols-40, (iplane==2)?cols/2:cols+40, // Ticks
			      rows, 0., rows); // Wires
		...
		FOR(j, cols) {
			...
			h->SetBinContent(j+1 + cols + 80, i+1, content);
			content = evt->as_vector()[iplane-1].pixel(i, j);
			h->SetBinContent(cols + 40 - j, i+1, content);
		    }
		    if ( DOCSV )
			fprintf(outfcsv, "%.0f\n", content);
